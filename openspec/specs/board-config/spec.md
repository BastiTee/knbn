...
Defines the schema, validation rules, and dataclass representation for the per-board configuration stored in `settings.json` under the `board` key.
...
- **WHEN** an active status name is a single character
...
- **WHEN** a priority name is exactly 2 characters
