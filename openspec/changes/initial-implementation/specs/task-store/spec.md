...
The store SHALL provide functions to add a new task (appends to CSV), update a task by index (replaces row), delete a task by index (removes row), and load all tasks (returns list in file order).
