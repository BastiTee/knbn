...
**Reason**: Statuses are now user-configurable via `settings.json`; compile-time constants cannot reflect runtime configuration. The constants `STATUS_ACTIVE`, `STATUS_TERMINAL`, and `STATUS_VALUES` are removed from the model module.
**Migration**: Code that imported these constants SHALL be updated to read from `BoardConfig` (loaded via `load_board_config()` from `knbn.config`). `BoardConfig.active_statuses`, `BoardConfig.terminal_statuses`, and `BoardConfig.all_statuses()` are the replacements.

### Requirement: Priority enumeration
**Reason**: Priorities are now user-configurable via `settings.json`; the hardcoded `PRIORITY_VALUES` constant cannot reflect runtime configuration.
**Migration**: Code that imported `PRIORITY_VALUES` SHALL use `BoardConfig.priorities` instead. Index-based promote/demote logic (`PRIORITY_VALUES.index(...)`) SHALL use `BoardConfig.priorities.index(...)`.

### Requirement: Default category list
**Reason**: Categories are now user-configurable; `DEFAULT_CATEGORIES` as a compile-time constant is replaced by `BoardConfig.categories`.
**Migration**: Code that imported `DEFAULT_CATEGORIES` SHALL use `[c.name for c in board_config.categories]` instead.
...
- **THEN** the returned string matches the pattern `YYYY-MM-DD HH:MM`
